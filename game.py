...
random.shuffle(deck)
donedeck = []
# deck = [Card(5), Card(11), Card(2), Card(3), Card(10), Card(10), Card(10), Card(4), Card(3), Card(1), Card(7)]
# print("deck: ", deck)
bet = 5

def getaction(hand, dealervalue):
    # if(len(hand.getCards()) == 2 and Card(1) not in hand.getCards() and (hand.getSum() == 16 and dealervalue in [9, 10, 1] or hand.getSum() == 15 and dealervalue == 10)):
    #     return "surrender"
    if hand.getCards()[0] == hand.getCards()[1]:  # pair splitting
        # print("Your hand contains a pair!")
        cardvalue = hand.getCards()[0].bjValue()
        if cardvalue == 1 or cardvalue == 8:
            # print("Split pair!")
            return "split"
        elif (cardvalue == 9 and dealervalue in [2, 3, 4, 5, 6, 8, 9]) or (cardvalue == 7 and 1<dealervalue<8) or (cardvalue == 6 and 2<dealervalue<7) or (2<=cardvalue<=3 and 4<=dealervalue<=7):
            # print("Split pair!")
            return "split"
        elif(cardvalue == 6 and dealervalue==2) or (1<cardvalue<4 and 1<dealervalue<4) or (cardvalue == 4 and 4<dealervalue<7):
            # print("Split if DAS is offered, otherwise Don't split!")
            return "split"
        else:
            pass
            # print("Don't split!")
    ...
    if(Card(1) in hand.getCards() and len(hand.getCards())==2):  # soft totals
        ...
        if (othercard >= 9) or (othercard == 8 and dealervalue != 6) or (othercard == 7 and 6<dealervalue<9):
            # print("Stand!")
            return "stand"
        if (1<othercard<7 and 4<dealervalue<7) or (3<othercard<7 and dealervalue == 4) or (othercard == 6 and dealervalue == 3):
            # print("Double if allowed, otherwise hit!")
            return "double"
     ...
     #  hard totals
    if (hand.getSum() == 10 and 1<dealervalue<=9)or hand.getSum() == 11 or (hand.getSum()==9 and 2<dealervalue<7):
        # print("You have to Double if allowed, otherwise hit!")
        return "double"
    elif (hand.getSum() >= 13 and 1<dealervalue<7) or (hand.getSum() == 12 and 3<dealervalue<7) or (hand.getSum()>=17):
...
def playround():
    ...
    while get_cards_sum(dealercards) < 17: #Dealer keeps hitting until 17 or higher
        ...
        for hand in playedhands:
            ...
            elif hand == Hand([Card(1), Card(10)], 0) and get_cards_sum(dealercards) != 21:
                # print("blackjack!")
